...
const CadResult = ({ data, isGenerating, generationStatus }: { data: any; isGenerating?: boolean; generationStatus?: string }) => {
  // The backend returns the gltf_url in the response
  // It should be a path like /api/v1/cad/download/model_xxxxx.gltf
  const gltfUrl = data?.gltf_url || null;
  
  // Debug logging
  if (gltfUrl) {
    console.log('🔵 ThreeViewer received gltfUrl:', gltfUrl);
  }
  ...
  return (
    <div>
      <SectionHeader 
        ...
        badge={gltfUrl ? 'Completed' : 'Running'} 
        badgeType={gltfUrl ? 'green' : 'blue'} 
      />
      <ThreeViewer
        modelUrl={gltfUrl}
...
const PhysicsResult = ({ data }: { data: any }) => {
  ...
  const downloadPhysicsReport = () => {
    const timestamp = new Date().toLocaleString();
    const safetyStatus = sf >= 2 ? 'Safe ✓' : sf >= 1 ? 'Borderline ⚠' : '❌ Unsafe';
...
CRITICAL METRICS
─────────────────────────────────────────────────────────────────
Maximum Stress:           ${data.max_stress_mpa?.toFixed(2) || 'N/A'} MPa
Safety Factor:            ${sf.toFixed(2)}
Safety Status:            ${safetyStatus}
Material Used:            ${data.material_used || 'N/A'}
Stress Load Percentage:   ${pct.toFixed(1)}% (${data.max_stress_mpa?.toFixed(1)} / 300 MPa)
...
Stress Distribution:
  - Maximum Stress:       ${data.max_stress_mpa?.toFixed(2) || 'N/A'} MPa
  - Stress Level:         ${pct > 70 ? 'HIGH RISK' : pct > 40 ? 'MODERATE' : 'LOW'}
  - Safety Margin:        ${(100 - pct).toFixed(1)}%

INTERPRETATION
─────────────────────────────────────────────────────────────────
${sf >= 2 ? '✅ SAFE: Structure has adequate safety margin (>2.0)' : sf >= 1 ? '⚠️  BORDERLINE: Safety factor is borderline (1.0-2.0). Consider design optimization.' : '❌ UNSAFE: Safety factor below 1.0. Design modification required.'}

Material Properties:
  - Yield Strength:       ~276 MPa (typical aluminum alloy)
  - Theoretical Limit:    ${(300 / sf).toFixed(1)} MPa with applied load

RECOMMENDATIONS
─────────────────────────────────────────────────────────────────
${sf < 1.5 ? '1. Consider increasing material thickness or switching to stronger alloy' : '1. Current design is acceptable for intended application'}
${pct > 70 ? '2. High stress concentration detected - review stress concentration points' : '2. Stress levels are within acceptable range'}
3. Perform thermal analysis if operating temperature exceeds 25°C
4. Validate simulation with physical testing before production

NOTES
─────────────────────────────────────────────────────────────────
- Simulation assumes linear elastic behavior
- Results are based on PINN (Physics-Informed Neural Network) analysis
- For production use, perform experimental validation
- This report is generated by InventAI Physics Agent
...
END OF REPORT
    ...
    link.href = url;
    link.download = `physics_report_${new Date().getTime()}.txt`;
...
function ProjectDashboardInner({ params, searchParams }: { params: Promise<{ id: string }>, searchParams: Promise<{ idea?: string }> }) {
        ...
        for (const line of lines) {
          if (line.startsWith('data: ')) {
            try {
              ...
              updateAgent(key, {
                status: d.status || '',
                // Persist partial data if it contains useful fields (e.g. gltf_url, physics data, etc)
                data: (d.gltf_url || d.id || d.novelty_score !== undefined || d.max_stress_mpa !== undefined || d.safety_factor !== undefined) ? d : lastData,
              });
            } catch { }
          }
        }
  ...
  // Build tab list with live status dots for the navbar
  const navTabs = TABS.map(t => {
    const ag = agents[t.key as keyof typeof agents];
    const dot = ag?.done && !ag?.error ? 'green' : ag?.error ? 'red' : null;
    return { key: t.key, label: t.label, icon: t.icon, dot } as const;
  });
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh', color: '#fff', fontFamily: '"Google Sans"' }}>
      ...
      {/* Content */}
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '100px 24px 28px', position: 'relative', zIndex: 5 }}>

        {/* ── Overview Tab ── */}
        {tab === 'overview' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {/* Top: Large CAD Viewer (full width) */}
            <div style={{ width: '100%' }}>
              {agents.cad.done || agents.cad.status ? (
                <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '16px', minHeight: '700px' }}>
                  <CadResult data={agents.cad.data} isGenerating={!agents.cad.done} generationStatus={agents.cad.status} />
                </div>
              ) : (
                <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '16px', minHeight: '700px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94A3B8' }}>
                  <Spinner /> Loading CAD generation...
                </div>
              )}
            </div>

            {/* Middle: Agent Pipeline Orchestrator */}
            <Card>
